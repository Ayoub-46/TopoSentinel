...
from .runner import FederatedExperiment
...
class ParallelFederatedExperiment(FederatedExperiment):
    ...
    def run(self):
        ...
        for round_idx in range(fl_cfg['num_rounds']):
            print(f"\n--- Round {round_idx}/{fl_cfg['num_rounds']} ---")
            ...
            for update in client_updates:
                if update: # Check if the task was successful
                    self.server.receive_update(update['weights'], update['num_samples'])
